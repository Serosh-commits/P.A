...
private:
    ...
    void scanProcesses()
    {
        ...
        while ((entry = readdir(dir)) != nullptr)
        {
            ...
            iss >> info.pid >> cmd >> info.state >> info.ppid >> dummy >> dummy >> dummy >> dummy >> 
                dummy >> dummy >> dummy >> dummy >> dummy >> info.utime >> info.stime >> dummy >> 
                dummy >> info.num_threads >> dummy >> dummy >> starttime >> dummy >> dummy >> dummy >> 
                dummy >> dummy >> dummy >> dummy >> dummy >> dummy >> dummy >> dummy >> dummy >> 
                dummy >> dummy >> dummy >> dummy >> dummy >> dummy >> info.priority >> info.nice;
            ...
            if (net_file)
            {
                std::getline(net_file, line); // Skip header
                std::getline(net_file, line); // Skip header
        ...
        // Second pass: identify orphans (processes with PPID=1 that are not init)
        for (auto &proc : processes)
        {
            proc.is_orphan = (proc.ppid == 1 && proc.pid != 1);
        }
        ...
        while (iss >> token)
        {
            ...
            if (std::regex_match(token, match, regex))
            {
                ...
                if (key == "pid" || key == "ppid" || key == "state" || key == "cmd" || 
                    key == "cpu" || key == "mem" || key == "age" || key == "zombie" || key == "orphan")
                {
    ...
    bool matchesFilter(const ProcessInfo &proc, const Filter &filter)
    {
        ...
        else if (filter.key == "zombie")
        {
            if (filter.op == ":" && filter.value == "true" && proc.is_zombie)
                return true;
            if (filter.op == ":" && filter.value == "false" && !proc.is_zombie)
                return true;
        }
        else if (filter.key == "orphan")
        {
            if (filter.op == ":" && filter.value == "true" && proc.is_orphan)
                return true;
            if (filter.op == ":" && filter.value == "false" && !proc.is_orphan)
                return true;
        }
        return false;
    }
    ...
    void logProcesses()
    {
        if (!log_file.is_open())
        {
            ...
            log_file << "Timestamp,PID,PPID,State,Cmd,Mem%,CPU%,IO R (KB/s),IO W (KB/s),RChar (KB),WChar (KB),Shared (KB),Private (KB),FD,Threads,CtxtSw,Age (h),Priority,Nice,CPUs,Net R (KB/s),Net W (KB/s),IsZombie,IsOrphan\n";
        }
        ...
        if (log_file)
        {
            ...
            for (const auto &proc : processes)
            {
                log_file << ts << "," << proc.pid << "," << proc.ppid << "," << proc.state << "," 
                         ...
                         << (proc.is_zombie ? "true" : "false") << "," << (proc.is_orphan ? "true" : "false") << "\n";
            }
            log_file.flush();
        }
    }
    ...
    void sortProcesses(std::string criterion)
    {
        ...
        else if (criterion == "zombie")
        {
            std::sort(processes.begin(), processes.end(), [](const ProcessInfo &a, const ProcessInfo &b) {
                return a.is_zombie > b.is_zombie;
            });
        }
        else if (criterion == "orphan")
        {
            std::sort(processes.begin(), processes.end(), [](const ProcessInfo &a, const ProcessInfo &b) {
    ...
    void displayHeader()
    {
        ...
        mvwprintw(win, 1, 0, "F4: Filter | F5: Tree/List | F6: Sort | F9: Kill | Z: Zombie/Orphan | L: Log | Q: Quit");
    ...
    void displayTree(pid_t pid, int depth, int &line, int max_lines, int max_cols)
    {
        ...
        if (line >= scroll_offset && line < max_lines + scroll_offset)
        {
            ...
            int color = 1; // Default color
            if (it->second.is_zombie)
                color = 3; // Yellow for zombies
            else if (it->second.is_orphan)
                color = 4; // Blue for orphans
            else if (it->second.cpu_usage > 50.0)
                color = 2; // Red for high CPU
            ...
            mvwprintw(win, line - scroll_offset + 3, 0, 
                      "%s%-5d %-5d %c %6.2f %6.2f %6.2f %6.2f %6lu %6lu %6lu %6lu %4lu %4ld %6lu %5.2f %3ld %3ld %-6s %6.2f %6.2f %-20s %s %s",
                      indent.c_str(), it->second.pid, it->second.ppid, it->second.state, 
                      it->second.mem_usage, it->second.cpu_usage, it->second.io_read_rate, 
                      it->second.io_write_rate, it->second.rchar/1024, it->second.wchar/1024,
                      it->second.shared_clean, it->second.private_dirty, it->second.fd_count, 
                      it->second.num_threads, it->second.voluntary_ctxt_switches, it->second.process_age, 
                      it->second.priority, it->second.nice, it->second.cpus_allowed_list.c_str(), 
                      it->second.net_rx_rate, it->second.net_tx_rate, cmd.c_str(),
                      it->second.is_zombie ? "Z" : "", it->second.is_orphan ? "O" : "");
    ...
    void displayProcesses(int max_lines, int /* max_cols */)
    {
        ...
        for (const auto &proc : processes)
        {
            ...
            wattron(win, COLOR_PAIR(color));
            mvwprintw(win, line - scroll_offset + 3, 0, 
                      "%-5d %-5d %c %6.2f %6.2f %6.2f %6.2f %6lu %6lu %6lu %6lu %4lu %4ld %6lu %5.2f %3ld %3ld %-6s %6.2f %6.2f %-20s %s %s",
                      ...
                      proc.is_zombie ? "Z" : "", proc.is_orphan ? "O" : "");
            wattroff(win, COLOR_PAIR(color));
    ...
    void handleInput(int ch)
    {
        ...
        switch (ch)
        {
            ...
            case KEY_F(4):
            {
                char input[256] = {0};
                echo();
                mvwprintw(win, 0, 0, "Filter (e.g., pid:1234 cpu>50 zombie:true): ");
                wrefresh(win);
            ...
            case KEY_F(6):
                if (sort_criterion == "cpu")
                    sort_criterion = "mem";
                else if (sort_criterion == "mem")
                    sort_criterion = "io";
                else if (sort_criterion == "io")
                    sort_criterion = "net";
                else if (sort_criterion == "net")
                    sort_criterion = "zombie";
                else if (sort_criterion == "zombie")
                    sort_criterion = "orphan";
                else
                    sort_criterion = "cpu";
            ...
            case KEY_F(9):
            {
                if (selected_row >= 0 && selected_row < static_cast<int>(processes.size()))
                {
                    ...
                    if (proc.is_zombie)
                    {
                        ...
                        kill_parent = true;
                        target_pid = proc.ppid;
                    }
                    else if (proc.is_orphan)
                    {
                        prompt = "Orphan PID=" + std::to_string(proc.pid) + " Cmd=" + proc.cmd + 
                                " Kill? (y/n): ";
                    }
                    else
                    {
                        prompt = "Kill PID=" + std::to_string(proc.pid) + " Cmd=" + proc.cmd + 
                                "? (y/n): ";
                    }
                    ...
                    if (c == 'y' || c == 'Y')
                    {
                        ...
                        {
                            status_msg = "SIGTERM sent to PID " + std::to_string(target_pid);
                        }
                        else
                        {
                            status_msg = "Error: Failed to send SIGTERM to PID " + std::to_string(target_pid);
                        }
            ...
            case 'Z':
            {
                ...
                for (const auto &proc : processes)
                {
                    if (proc.is_zombie || proc.is_orphan)
                    {
                        filtered.push_back(proc);
                    }
                }
                processes = filtered;
                filters.clear();
                status_msg = "Filtered zombies and orphans";
                selected_row = 0;
                scroll_offset = 0;
                needs_redraw = true;
                break;
    ...
    void run()
    {
        while (true)
        {
            ...
            if (needs_redraw)
            {
                ...
                displayHeader();
                mvwprintw(win, 3, 0, 
                          "%-5s %-5s %s %6s %6s %6s %6s %6s %6s %6s %6s %4s %4s %6s %5s %3s %3s %-6s %6s %6s %-20s %s %s",
