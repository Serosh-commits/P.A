...
private:
    ...
    void scanProcesses()
    {
        ...
        while ((entry = readdir(dir)))
        {
            ...
            std::string cmd;
            uint64_t starttime;
            iss >> info.pid >> cmd >> info.state >> info.ppid >> dummy >> dummy >> dummy >> dummy >> dummy >> dummy >> dummy >> dummy >> dummy >> info.utime >> info.stime >> dummy >> dummy >> info.priority >> info.nice >> info.num_threads >> dummy >> starttime;
            if (cmd.length() >= 2)
            {
    ...
    void displayTree(pid_t pid, int depth, int &line, int max_lines, int max_cols)
    {
        ...
        if (line >= scroll_offset && line < max_lines + scroll_offset)
        {
            ...
            mvwprintw(win, line - scroll_offset + 3, 0, "%s%-5d %-5d %c %6.2f %6.2f %6.2f %6.2f %6llu %6llu %6llu %6llu %4llu %4ld %6llu %5.2f %3ld %3ld %-6s %6.2f %6.2f %-20s",
                      ...
                      (unsigned long long)(it->second.rchar / 1024), (unsigned long long)(it->second.wchar / 1024),
                      (unsigned long long)it->second.shared_clean, (unsigned long long)it->second.private_dirty, 
                      (unsigned long long)it->second.fd_count, it->second.num_threads,
    ...
    {
        (void)max_cols; // Unused parameter
        int line = 0;
        for (const auto &proc : processes)
        {
            if (line >= max_lines + scroll_offset)
            ...
            mvwprintw(win, line - scroll_offset + 3, 0, "%-5d %-5d %c %6.2f %6.2f %6.2f %6.2f %6llu %6llu %6llu %6llu %4llu %4ld %6llu %5.2f %3ld %3ld %-6s %6.2f %6.2f %-20s",
                      ...
                      (unsigned long long)(proc.rchar / 1024), (unsigned long long)(proc.wchar / 1024),
                      (unsigned long long)proc.shared_clean, (unsigned long long)proc.private_dirty, 
                      (unsigned long long)proc.fd_count, proc.num_threads,
    ...
    void handleInput(int ch)
    {
        ...
        switch (ch)
        {
            ...
            case KEY_F(4):
            {
                ...
                {
                    auto new_filters = parseFilters(s);
...
public:
    ...
    void run()
    {
        while (true)
        {
            ...
            {
                int line = 0;
                // Display all root processes (those whose parent doesn't exist in the process map)
                for (const auto &proc : processes)
                {
