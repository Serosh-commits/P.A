...
private:
    ...
    void displayTree(pid_t pid, int depth, int &line, int max_lines, int max_cols)
    {
        ...
        if (line >= scroll_offset && line < max_lines + scroll_offset)
        {
            ...
            mvwprintw(win, line - scroll_offset + 3, 0, "%s%-5d %-5d %c %6.2f %6.2f %6.2f %6.2f %6lu %6lu %6lu %6lu %4lu %4ld %6lu %5.2f %3ld %3ld %-6s %6.2f %6.2f %-20s",
    ...
    void displayProcesses(int max_lines, int /* max_cols */)
    {
        ...
        for (const auto &proc : processes)
        {
            ...
            wattron(win, COLOR_PAIR(color));
            mvwprintw(win, line - scroll_offset + 3, 0, "%-5d %-5d %c %6.2f %6.2f %6.2f %6.2f %6lu %6lu %6lu %6lu %4lu %4ld %6lu %5.2f %3ld %3ld %-6s %6.2f %6.2f %-20s",
    ...
    // New method to detect and handle zombies and orphans
    void detectAndHandleZombiesOrphans()
    {
        std::vector<ProcessInfo> zombies;
        std::vector<ProcessInfo> orphans;
        
        // Find zombies and orphans
        for (const auto &proc : processes)
        {
            if (proc.state == 'Z')
            {
                zombies.push_back(proc);
            }
            else if (proc.ppid == 1 && proc.pid != 1)
            {
                orphans.push_back(proc);
            }
        }
        ...
        if (!zombies.empty())
        {
            werase(win);
            displayHeader();
            mvwprintw(win, 3, 0, "ZOMBIE PROCESSES DETECTED:");
            mvwprintw(win, 4, 0, "PID     PPID    CMD");
            
            int line = 5;
            for (const auto &zombie : zombies)
            {
                std::string cmd = zombie.cmd;
                if (cmd.length() > 30)
                {
                    cmd = cmd.substr(0, 27) + "...";
                }
                mvwprintw(win, line++, 0, "%-8d %-8d %s", zombie.pid, zombie.ppid, cmd.c_str());
            }
            
            mvwprintw(win, line++, 0, " ");
            mvwprintw(win, line++, 0, "Zombie processes found! These are dead processes waiting for their parent to reap them.");
            mvwprintw(win, line++, 0, "Kill the parent process (PPID) to clean up zombies? (y/n): ");
            wrefresh(win);
            ...
            if (choice == 'y' || choice == 'Y')
            {
                for (const auto &zombie : zombies)
                {
                    if (kill(zombie.ppid, SIGTERM) == 0)
                    {
                        status_msg = "SIGTERM sent to parent PID " + std::to_string(zombie.ppid) + " of zombie " + std::to_string(zombie.pid);
                    }
                    else
                    {
                        status_msg = "Error: Failed to send SIGTERM to parent PID " + std::to_string(zombie.ppid);
                    }
                }
            }
            else
            {
                status_msg = "Zombie cleanup cancelled";
            }
        }
        ...
        if (!orphans.empty())
        {
            werase(win);
            displayHeader();
            mvwprintw(win, 3, 0, "ORPHAN PROCESSES DETECTED:");
            mvwprintw(win, 4, 0, "PID     CMD");
            
            int line = 5;
            for (const auto &orphan : orphans)
            {
                std::string cmd = orphan.cmd;
                if (cmd.length() > 30)
                {
                    cmd = cmd.substr(0, 27) + "...";
                }
                mvwprintw(win, line++, 0, "%-8d %s", orphan.pid, cmd.c_str());
            }
            
            mvwprintw(win, line++, 0, " ");
            mvwprintw(win, line++, 0, "Orphan processes found! These are processes whose parent has died.");
            mvwprintw(win, line++, 0, "Kill these orphan processes? (y/n): ");
            wrefresh(win);
            ...
            if (choice == 'y' || choice == 'Y')
            {
                for (const auto &orphan : orphans)
                {
                    ...
                    {
                        status_msg = "SIGTERM sent to orphan PID " + std::to_string(orphan.pid);
                    }
                    else
                    {
                        status_msg = "Error: Failed to send SIGTERM to orphan PID " + std::to_string(orphan.pid);
                    }
                }
            }
            else
            {
                status_msg = "Orphan cleanup cancelled";
            }
        }
        
        // If no zombies or orphans found
        if (zombies.empty() && orphans.empty())
        {
            status_msg = "No zombie or orphan processes found. System is clean!";
        }
    }
