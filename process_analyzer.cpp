...
private:
    ...
    bool logging_enabled = false, tree_view = true, needs_redraw = true;
    ...
    void scanProcesses()
    {
        ...
        while ((entry = readdir(dir)) != nullptr)
        {
            ...
            info.mem_usage = 0.0;
            info.cpu_usage = 0.0;
            info.io_read_rate = 0.0;
            info.io_write_rate = 0.0;
            info.process_age = 0.0;
            info.net_rx_rate = 0.0;
            info.net_tx_rate = 0.0;
            info.utime = 0;
            info.stime = 0;
            info.read_bytes = 0;
            info.write_bytes = 0;
            info.rchar = 0;
            info.wchar = 0;
            info.voluntary_ctxt_switches = 0;
            info.shared_clean = 0;
            ...
            // Parse stat file fields
            std::string temp_cmd;
            uint64_t starttime = 0;
            int dummy;
            
            iss >> info.pid >> temp_cmd >> info.state >> info.ppid;
            ...
            // Skip more fields
            for (int i = 0; i < 2; i++) iss >> dummy;
            
            iss >> starttime;
            
            // Skip more fields until priority and nice
            for (int i = 0; i < 13; i++) iss >> dummy;
            
            iss >> info.priority >> info.nice;
            
            // Clean up command name (remove parentheses)
            if (temp_cmd.length() > 2)
            {
                info.cmd = temp_cmd.substr(1, temp_cmd.size() - 2);
            }
            else
            {
                info.cmd = temp_cmd;
            ...
            // Read /proc/pid/smaps for memory info
            ...
            if (fd_dir)
            {
                ...
                {
                    if (fd_entry->d_type != DT_DIR && strcmp(fd_entry->d_name, ".") != 0 && strcmp(fd_entry->d_name, "..") != 0)
                    {
                        info.fd_count++;
                    }
                }
            ...
            // Read network statistics (simplified)
            path = "/proc/" + std::to_string(pid) + "/net/dev";
            std::ifstream net_file(path);
            if (net_file)
            {
                std::string header1, header2;
                std::getline(net_file, header1);
                std::getline(net_file, header2);
        ...
        if (prev_total_jiffies > 0)
        {
            ...
            if (delta_total > 0)
            {
                system_cpu_usage = 100.0 * static_cast<double>(work_jiffies - prev_work_jiffies) / delta_total;
            }
        }
        
        // Calculate per-process CPU usage and I/O rates
        for (auto &proc : processes)
        {
            auto it = std::find_if(prev_processes.begin(), prev_processes.end(),
                                   [&](const ProcessInfo &p) { return p.pid == proc.pid; });
    ...
    std::vector<Filter> parseFilters(const std::string &input)
    {
        std::vector<Filter> filters;
        if (input.empty())
        {
        ...
        while (iss >> token)
        {
            ...
            if (std::regex_match(token, match, regex))
            {
                ...
                if (key == "pid" || key == "ppid" || key == "state" || key == "cmd" || 
                    key == "cpu" || key == "mem" || key == "age")
                {
    ...
    bool matchesFilter(const ProcessInfo &proc, const Filter &filter)
    {
        ...
        else if (filter.key == "ppid")
        {
            ...
            {
                pid_t val = std::stoi(filter.value);
                return (filter.op == ":" && proc.ppid == val);
            }
            catch (...)
            {
        ...
        {
            return (filter.op == ":" && proc.state == filter.value[0]);
        }
        else if (filter.key == "cmd")
        {
            ...
            return (filter.op == ":" && cmd.find(val) != std::string::npos);
        }
        else if (filter.key == "cpu")
        {
            try
            {
                ...
                if (filter.op == ">") return proc.cpu_usage > val;
                if (filter.op == "<") return proc.cpu_usage < val;
                if (filter.op == ":") return std::abs(proc.cpu_usage - val) < 0.01;
            }
            catch (...)
            {
        ...
        else if (filter.key == "mem")
        {
            ...
            {
                double val = std::stod(filter.value);
                if (filter.op == ">") return proc.mem_usage > val;
                if (filter.op == "<") return proc.mem_usage < val;
                if (filter.op == ":") return std::abs(proc.mem_usage - val) < 0.01;
            }
            catch (...)
            {
        ...
        else if (filter.key == "age")
        {
            ...
            {
                double val = std::stod(filter.value);
                if (filter.op == ">") return proc.process_age > val;
                if (filter.op == "<") return proc.process_age < val;
                if (filter.op == ":") return std::abs(proc.process_age - val) < 0.01;
            }
            catch (...)
            {
        ...
        if (log_file)
        {
            ...
            for (const auto &proc : processes)
            {
                log_file << ts << "," << proc.pid << "," << proc.ppid << "," << proc.state << "," 
                        << proc.cmd << "," << proc.mem_usage << "," << proc.cpu_usage << "," 
                        << proc.io_read_rate << "," << proc.io_write_rate << "," << proc.rchar / 1024 << "," 
                        << proc.wchar / 1024 << "," << proc.shared_clean << "," << proc.private_dirty << "," 
                        << proc.fd_count << "," << proc.num_threads << "," << proc.voluntary_ctxt_switches << "," 
                        << proc.process_age << "," << proc.priority << "," << proc.nice << "," 
                        << proc.cpus_allowed_list << "," << proc.net_rx_rate << "," << proc.net_tx_rate << "\n";
            }
            log_file.flush();
        }
    }
    ...
    void displayHeader()
    {
        ...
        mvwprintw(win, 0, 0, "System CPU: %.2f%% Mem: %.2f%% Uptime: %.2f h | Cores: %d | %s | Sort: %s | Filter: %s",
                  ...
                  logging_enabled ? "Logging: ON" : "Logging: OFF", 
                  sort_criterion.empty() ? "None" : sort_criterion.c_str(),
    ...
    void displayTree(pid_t pid, int depth, int &line, int max_lines, int max_cols)
    {
        ...
        if (line >= scroll_offset && line < max_lines + scroll_offset)
        {
            ...
            int color = (it->second.state == 'Z') ? 3 : 
                       (it->second.ppid == 1 && it->second.pid != 1) ? 4 : 
                       (it->second.cpu_usage > 50.0) ? 2 : 1;
            ...
            mvwprintw(win, line - scroll_offset + 3, 0, 
                     "%s%-5d %-5d %c %6.2f %6.2f %6.2f %6.2f %6llu %6llu %6llu %6llu %4llu %4ld %6llu %5.2f %3ld %3ld %-6s %6.2f %6.2f %-20s",
                     indent.c_str(), it->second.pid, it->second.ppid, it->second.state, 
                     it->second.mem_usage, it->second.cpu_usage,
                     it->second.io_read_rate, it->second.io_write_rate, 
                     it->second.rchar / 1024, it->second.wchar / 1024,
                     it->second.shared_clean, it->second.private_dirty, 
                     it->second.fd_count, it->second.num_threads,
                     it->second.voluntary_ctxt_switches, it->second.process_age, 
                     it->second.priority, it->second.nice,
                     it->second.cpus_allowed_list.c_str(), 
                     it->second.net_rx_rate, it->second.net_tx_rate, cmd.c_str());
    ...
    {
        for (int line = 0; line < static_cast<int>(processes.size()); line++)
        {
            if (line >= max_lines + scroll_offset || line < scroll_offset)
            {
                continue;
            ...
            const auto &proc = processes[line];
            std::string cmd = proc.cmd;
            if (cmd.length() > 20)
            {
                cmd = cmd.substr(0, 17) + "...";
            ...
            int color = (proc.state == 'Z') ? 3 : 
                       (proc.ppid == 1 && proc.pid != 1) ? 4 : 
                       (proc.cpu_usage > 50.0) ? 2 : 1;
            ...
            mvwprintw(win, line - scroll_offset + 3, 0, 
                     "%-5d %-5d %c %6.2f %6.2f %6.2f %6.2f %6llu %6llu %6llu %6llu %4llu %4ld %6llu %5.2f %3ld %3ld %-6s %6.2f %6.2f %-20s",
    ...
    void handleInput(int ch)
    {
        ...
        int total_lines = processes.empty() ? 0 : static_cast<int>(processes.size());
        status_msg.clear();
        ...
        switch (ch)
        {
            ...
            case KEY_F(6):
                sort_criterion = (sort_criterion == "cpu") ? "mem" : 
                                (sort_criterion == "mem") ? "io" : 
                                (sort_criterion == "io") ? "net" : "cpu";
            ...
            case KEY_F(9):
            {
                if (selected_row >= 0 && selected_row < static_cast<int>(processes.size()))
                {
                    ...
                    if (proc.state == 'Z')
                    {
                        prompt = "Zombie PID=" + std::to_string(proc.pid) + " Cmd=" + proc.cmd + 
                                " Kill parent PPID=" + std::to_string(proc.ppid) + "? (y/n): ";
                        kill_parent = true;
                    }
            ...
            case 'Z':
            {
                ...
                processes = filtered;
                filters.clear();
                status_msg = "Filtered zombies and orphans (found " + std::to_string(filtered.size()) + ")";
                selected_row = 0;
                scroll_offset = 0;
                needs_redraw = true;
                break;
...
public:
    ProcessAnalyzer()
    {
        ...
        initscr();
        start_color();
        init_pair(1, COLOR_GREEN, COLOR_BLACK);   // Normal processes
        init_pair(2, COLOR_RED, COLOR_BLACK);     // High CPU processes
        init_pair(3, COLOR_YELLOW, COLOR_BLACK);  // Zombie processes
        init_pair(4, COLOR_BLUE, COLOR_BLACK);    // Orphan processes
        ...
        while (true)
        {
            ...
            if (needs_redraw)
            {
                ...
                mvwprintw(win, 3, 0, "%-5s %-5s %s %6s %6s %6s %6s %6s %6s %6s %6s %4s %4s %6s %5s %3s %3s %-6s %6s %6s %-20s",
                          "PID", "PPID", "S", "Mem%", "CPU%", "IO R", "IO W", "RChar", "WChar", 
                          "Shared", "Priv", "FD", "Thrd", "CtxtSw", "Age", "Pri", "Nice", "CPUs", 
                          "Net R", "Net W", "Cmd");
            }
            ...
            while (std::chrono::duration_cast<std::chrono::seconds>(
                       std::chrono::steady_clock::now() - start).count() < poll_interval)
            {
                int ch = getch();
                if (ch != ERR)
                {
