...
long num_threads,priority,nice;
...
bool logging_enabled=false,tree_view=true,needs_full_redraw=true;
std::string sort_criterion,status_message;
...
status_iss>>info.voluntary_ctxt_switches;
}
else if(key=="Cpus_allowed_list:"){
status_iss>>info.cpus_allowed_list;
}
}
status_file.close();
}
...
io_iss>>key;
if(key=="read_bytes:"){
io_iss>>info.read_bytes;
}
else if(key=="write_bytes:"){
io_iss>>info.write_bytes;
}
else if(key=="rchar:"){
io_iss>>info.rchar;
}
...
uint64_t rx_bytes,dummy_rx,tx_bytes;
...
f.push_back({key,op,value});
}
else{
status_message="Invalid filter key: "+key;
return {};
}
}
else{
status_message="Invalid filter format: "+token;
return {};
}
}
return f;
...
bool matchesFilter(const ProcessInfo& proc,const Filter& f){
if(f.key=="pid"){
try{
pid_t v=std::stoi(f.value);
if(f.op==":"&&proc.pid==v){
return true;
}
}
catch(...){
status_message="Invalid PID: "+f.value;
return false;
}
}
else if(f.key=="ppid"){
...
status_message="Invalid PPID: "+f.value;
return false;
}
}
else if(f.key=="state"){
if(f.op==":"&&proc.state==f.value[0]){
return true;
}
}
else if(f.key=="cmd"){
std::string lower_cmd=proc.cmd;
std::string lower_value=f.value;
std::transform(lower_cmd.begin(),lower_cmd.end(),lower_cmd.begin(),::tolower);
std::transform(lower_value.begin(),lower_value.end(),lower_value.begin(),::tolower);
if(f.op==":"&&lower_cmd.find(lower_value)!=std::string::npos){
return true;
}
}
else if(f.key=="cpu"){
try{
double v=std::stod(f.value);
if(f.op==">"&&proc.cpu_usage>v){
return true;
}
if(f.op=="<"&&proc.cpu_usage<v){
return true;
}
if(f.op==":"&&std::abs(proc.cpu_usage-v)<0.01){
return true;
}
}
catch(...){
status_message="Invalid CPU value: "+f.value;
return false;
}
}
else if(f.key=="mem"){
try{
double v=std::stod(f.value);
if(f.op==">"&&proc.mem_usage>v){
return true;
}
if(f.op=="<"&&proc.mem_usage<v){
return true;
}
if(f.op==":"&&std::abs(proc.mem_usage-v)<0.01){
return true;
}
}
catch(...){
status_message="Invalid Mem value: "+f.value;
return false;
}
}
else if(f.key=="age"){
try{
double v=std::stod(f.value);
if(f.op==">"&&proc.process_age>v){
return true;
}
if(f.op=="<"&&proc.process_age<v){
return true;
}
if(f.op==":"&&std::abs(proc.process_age-v)<0.01){
return true;
}
}
catch(...){
status_message="Invalid Age value: "+f.value;
return false;
}
}
return false;
...
std::vector<ProcessInfo> f;
f.reserve(processes.size());
...
status_message="Filter applied: "+s;
}
else{
status_message=status_message.empty()?"Invalid filter format":status_message;
}
}
else{
filters.clear();
...
processes=temp;
filters.clear();
status_message="Filtered zombies and orphans";
selected_row=0;
scroll_offset=0;
needs_full_redraw=true;
break;
